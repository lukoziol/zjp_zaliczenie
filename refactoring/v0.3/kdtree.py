...
class KDTree:
    ...
    def __make_split(self, tree_level, index, xsorted, ysorted):
        ...
        # logowanie
        self.__logger(1, tree_level, 'Level: {:d} index: {:d} size: {:d}'.format(tree_level, index, size))
        ...
        self.__logger(2, tree_level, 'Split point at {:d}: {}'.format(mid, mid_point))
    ...
    def __point_in_range(self, point):
        ...
        if self.range[0][0] <= point[0] <= self.range[1][0] and self.range[0][1] <= point[1] <= self.range[1][1]:
            in_range = True
        self.__logger(5, 0, 'Point {} is{} in range {}'.format(point, (' not', '')[in_range], self.range))
        return in_range
    ...
    def __search_range_recursive(self, point_index, depth):
        ...
        if point[axis] < self.range[0][axis]:
            ...
            self.__logger(3, depth, 'Point {} X is in {}'.format(point, (self.range[0][axis], self.range[1][axis])))
